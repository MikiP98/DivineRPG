...
public class BlockMod extends Block {
    ...
    public BlockMod(Settings settings) {super(settings);}
    ...
    public BlockMod(MapColor color, float hardness, float resistance) {
        super(Settings.copy(Blocks.STONE).mapColor(color).strength(hardness, resistance));
    }
    ...
    // For BlockModLamp
    public BlockMod(MapColor color, float hardness, float resistance, BlockSoundGroup sound, Instrument instrument, int luminance) {
        super(Settings.create().mapColor(color).requiresTool().strength(hardness, resistance).sounds(sound).instrument(instrument).luminance((state) -> luminance));
    }
    //Hard blocks like obsidian, but luminous
    public BlockMod(MapColor color, int luminance) {
        super(Settings.copy(Blocks.OBSIDIAN).mapColor(color).pistonBehavior(PistonBehavior.BLOCK).luminance((state) -> luminance));
    }
}